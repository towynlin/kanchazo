...
export async function getOrCreateIcalToken(userId: string): Promise<string> {
  const existing = await db.select().from(icalTokens).where(eq(icalTokens.userId, userId)).limit(1);
...
export async function findUserByIcalToken(token: string): Promise<string | null> {
  const rows = await db.select().from(icalTokens).where(eq(icalTokens.tokenHash, token)).limit(1);
